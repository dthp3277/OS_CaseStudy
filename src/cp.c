  ...
  if (ok && argc - optind >= 2) {
    ...
    if (stat(argv[optind], &sb) == 0) {
      if (S_ISDIR(sb.st_mode)) {
        printf("\x1b[1;33mFolder '%s' has been successfully copied to '%s'\n", argv[optind], argv[optind + 1]);
      } else {
        printf("\x1b[1;33mFile '%s' has been successfully copied to '%s'\n", argv[optind], argv[optind + 1]);
      }
    }
    else {
      printf("\x1b[1;33mContents at '%s' has been successfully copied to '%s'\n", argv[optind], argv[optind + 1]);
    }
  }
