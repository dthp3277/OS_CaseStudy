          ...
          if (ok) {
            ...
            if (stat(source, &sb) == 0) {
              if (S_ISDIR(sb.st_mode)) {
                printf("\x1b[1;33mFolder '%s' has been successfully moved to '%s'\n", source, target_directory);
              } else {
                printf("\x1b[1;33mFile '%s' has been successfully moved to '%s'\n", source, target_directory);
              }
            }
            else {
              printf("\x1b[1;33mContents at '%s' has been successfully moved to '%s'\n", source, target_directory);
            }
          }
        }
    }
